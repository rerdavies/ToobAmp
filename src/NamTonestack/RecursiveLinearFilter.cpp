...
void recursive_linear_filter::Base::_PrepareBuffers(const size_t numChannels, const size_t numFrames)
{
  ...
  this->tonestack_dsp::DSP::_PrepareBuffers(numChannels, numFrames);
  if (newChannels)
  {
...
double recursive_linear_filter::Biquad::GetFrequencyResponse(float w)
{
    std::complex<double> z0 = std::exp(std::complex<double>(0,w));


    std::complex<double> numerator = mInputCoefficients[0];
    std::complex<double> zN = z0;
    for (size_t i = 1; i < mInputCoefficients.size(); ++i)
    {
        numerator += zN*mInputCoefficients[i];
        zN *= z0;
    }
    std::complex<double> denominator = 1.0;
    zN = z0;
    for (size_t i = 1; i < mOutputCoefficients.size(); ++i)
    {
        denominator -= zN*mOutputCoefficients[i]; // coefficients are sign-flipped.
        zN *= z0;
    }
