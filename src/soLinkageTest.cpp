...
int main(int argc, char **argv)
{
    ...
    {
        std::filesystem::path soName = std::filesystem::path(argv[0]).parent_path() / "ToobAmpUI.so";
        ...
        void *entryPoint = dlsym(handle, "lv2ui_descriptor");
        if (entryPoint == nullptr)
        {
            cerr << "ToobAmpUI Entry point not found." << endl;
            return EXIT_FAILURE;
        }
